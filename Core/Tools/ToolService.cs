...
public static class ToolService
{
    ...
    public static Tool SelectTool(Machine machine)
    {
        if (_toolsForm == null)
        {
            ...
            _toolsForm.SaveTools += (sender, args) => SaveTools();
        }
        _machine = machine;
        _toolsForm.Text = $"Инструмент для станка {machine} [{GetToolFileName(machine)}]";
    ...
    private static readonly Dictionary<Machine, List<Tool>> Tools = new();
    ...
    private static List<Tool> GetTools(Machine machine)
    {
        ...
        if (File.Exists(file))
        {
            tools = File.ReadAllLines(file)
                ...
                {
                    Number = ToInt(p[0]),
                    Type = (ToolType)ToInt(p[1]),
                    Diameter = ToDouble(p[2]),
                    Thickness = ToDouble(p[3]),
                    Name = p[4],
                })
                .ToList();
        }
    ...
    private static void SaveTools()
    {
        var file = GetToolFileName(_machine);
        try
        {
            var lines = Tools[_machine].Select(p => $"{p.Number};{(int)p.Type};{p.Diameter};{p.Thickness};{p.Name}");
    ...
    private static void WriteErrorParsing(string par) => Acad.Write($"Ошибка преобразования числового параметра {par}");
    ...
    //            k => new
    //            {
    //                result = double.TryParse(k, NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture, out var value), 
    //                value
    //            }))
