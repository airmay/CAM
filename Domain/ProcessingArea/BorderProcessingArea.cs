...
namespace CAM.Domain
{
	public class BorderProcessingArea : ProcessingArea
	{
		...
		public static void SetupBorders(List<BorderProcessingArea> borders)
		{
			...
			while ((border = borders.Find(p => !processed.Contains(p) && p.OuterSide != Side.None) ?? borders.Find(p => !processed.Contains(p))) != null)
			{
		...
		private static void CalcBordersChain(BorderProcessingArea border, Corner corner, List<BorderProcessingArea> borders, ISet<BorderProcessingArea> processed)
		{
			var startPoint = border.Curve.GetPoint(corner);
			var point = startPoint;
			BorderProcessingArea nextBorder;
			while ((nextBorder = borders.SingleOrDefault(p => !processed.Contains(p) && p != border && (p.Curve.StartPoint == point || p.Curve.EndPoint == point))) != null)
			{
		...
		private static bool CalcIsExactly(BorderProcessingArea border, Corner corner, BorderProcessingArea nextBorder, Corner nextCorner)
		{
			switch (border.Curve)
			{
				case Line line:
