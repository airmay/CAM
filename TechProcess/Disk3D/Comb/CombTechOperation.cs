...
namespace CAM.Disk3D
{
 ...
 //               var s = DbSurface.CreateOffsetSurface(surface, 2, true);
            ...
            //    UnitDir = Vector3d.YAxis
            //};
            //DbSurface surface = null;
            //var boundsModel = new Extents3d();
            //foreach (var entity in TechProcess.ProcessingArea.Select(p => p.ObjectId.QOpenForRead<Entity>()).OrderBy(p => p.GeometricExtents.MinPoint.Y))
            //{
            //    surface = entity as DbSurface;
            //    if (entity is Region region)
            //    {
            //        if (Math.Abs(region.Normal.Z) < Tolerance.Global.EqualPoint)
            //            continue;
            //        surface = new PlaneSurface();
            //        ((PlaneSurface)surface).CreateFromRegion(region);
            //    }
            //    if (surface == null)
            //        throw new Exception($"Объект типа {entity.GetType()} не может быть обработан (1)");

            //    var bounds = surface.GeometricExtents;
            //    ray.BasePoint = new Point3d(bounds.MinPoint.X + (bounds.MaxPoint.X - bounds.MinPoint.X) / 2, 0, 0);
            //    var curves = surface.ProjectOnToSurface(ray, Vector3d.ZAxis);
            //    if (curves.Length > 1)
            //        throw new Exception($"Объект не может быть обработан (2)");
            //    surfaceDict.Add(surface, (Curve)curves[0]);
            //    //curves = curves.Concat(surface.ProjectOnToSurface(line, Vector3d.ZAxis).Cast<Curve>());
            ...
            //    //}
            //    boundsModel.AddExtents(bounds);
            //}




            //var line = NoDraw.Line(new Point3d(500, -500, 0), new Point3d(1000, 500, 0));
            //line.ColorIndex = 2;
            ...
            //foreach (var dBObject in TechProcess.ProcessingArea.Select(p => p.ObjectId.QOpenForRead()))
            //{
            //    surface = dBObject as DbSurface;
            //    if (dBObject is Region region)
            //    {
            //        if (Math.Abs(region.Normal.Z) < Tolerance.Global.EqualVector)
            //            continue;
            //        surface = new PlaneSurface();
            //        ((PlaneSurface)surface).CreateFromRegion(region);
            //    }
            ...
            //    if (curves.Length > 1)
            //        throw new Exception($"Объект не может быть обработан (2)");
            //    var c = (Curve)curves[0];
            //    if (c.StartPoint.Y > c.EndPoint.Y)
            //        c.ReverseCurve();
            //    surfaceDict.Add(surface, c);
            //    //curves = curves.Concat(surface.ProjectOnToSurface(line, Vector3d.ZAxis).Cast<Curve>());
            ...
            */
            ...
            //Acad.Write(boundsModel.MinPoint.ToString() + " - " + boundsModel.MaxPoint.ToString());

            //var ray = new Ray
            //{
            //    BasePoint = new Point3d(500, 50, 0),
            //    UnitDir = Vector3d.ZAxis
            //};
            //var points = new Point3dCollection();
            //var line = NoDraw.Line(new Point3d(1000, -500, 0), new Point3d(1000, 500, 0));
            //line.ColorIndex = 2;
            //line.AddToCurrentSpace();
            //ray.IntersectWith(TechProcess.ProcessingArea[0].ObjectId.QOpenForRead<Entity>(), Intersect.OnBothOperands, new Plane(), points, IntPtr.Zero, IntPtr.Zero);
            //var p1 = TechProcess.ProcessingArea[0].ObjectId.QOpenForRead<Autodesk.AutoCAD.DatabaseServices.Surface>();
            //var p = TechProcess.ProcessingArea[0].ObjectId.QOpenForRead<Autodesk.AutoCAD.DatabaseServices.Region>();

            //var es = new DBObjectCollection();
            //p.Explode(es);
            //var plane = new Plane(new Point3d(1000, 0, 0), Vector3d.ZAxis);
            //p.IntersectWith(plane, Intersect.OnBothOperands, points, 0, 0);

            //ray.Dispose();

            //var ps = new PlaneSurface();
            //ps.CreateFromRegion(p);
            //var res = ps.ProjectOnToSurface(line, Vector3d.ZAxis);

            ////var point = new Point3d(500, 50, 0);
            ////var res = p.ProjectOnToSurface(line, Vector3d.ZAxis);
            //foreach (var item in res)
            //{
            //    item.ColorIndex = 6;
            //    item.AddToCurrentSpace();
            //}
            //var plane = ((Entity)es[0]).GetPlane();
            //var curve = line.GetProjectedCurve(plane, Vector3d.ZAxis);
            //curve.ColorIndex = 6;
            //curve.AddToCurrentSpace();
